...
def predict_crop():
    ...
    if not crop_model:
        return jsonify({
            'error': 'Model not available. Please install required dependencies: pip install scikit-learn pandas numpy joblib'
        }), 503
...
def get_available_crops():
    ...
    if not crop_model:
        return jsonify({
            'error': 'Model not available. Please install required dependencies.'
        }), 503
